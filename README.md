...
2. Optimize a max-Sharpe portfolio from the top-ranked candidates using mean-variance optimization (covariance matrix + SLSQP solver).
...
| `porto.py` | Pulls the top-N tickers from `sharpe_ratio_scan_results.csv`, computes the annualized covariance matrix of excess returns, and optimizes portfolio weights for maximum Sharpe ratio (long-only, bounded per-asset weight) using `scipy.optimize`. |
| `data_us10y.csv` | Historical US 10-Year Treasury yield data, used as the risk-free rate across all scripts. |
...
2. **Optimize a max-Sharpe portfolio from the top-N scan results:**
   ...
   python porto.py
   \`\`\`
   Automatically pulls the top `TOP_N` tickers (default 4) from `sharpe_ratio_scan_results.csv`, then outputs optimal weights, dollar allocation, expected annualized return/volatility, portfolio Sharpe ratio, and the asset correlation matrix.
...
- Optimization uses multiple random restarts (`N_RESTARTS`) to reduce the risk of converging on a local optimum.
- All optimizations here are in-sample — no out-of-sample or forward-test validation is included in this toolkit.
